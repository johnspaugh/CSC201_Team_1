...
(* 1-2 check Validity of DeclarationList
     1. NoDuplicate: DeclarationList --> 
               variable --> Bool
Fun NoDuplicate ((a:variable, b:Type):: decListTail)
     (c: variable) = 
...
val rec wholeAbsTypingTable = fn (decListHead :: decListTail : DeclarationList) =>
...
fun DetermineExpType (EDC1 (x)) = (fn (y:AbsTypingTable) => y(x)) |
        ...
        DetermineExpType (EDC4 (x1, x2, ODC1(AOp))) =
                                  (fn (y:AbsTypingTable) => DeclaredInt) |
        DetermineExpType (EDC5 (x1, x2, ODC2(ROp))) =
                                  (fn (y:AbsTypingTable) => DeclaredBool) |
        DetermineExpType (EDC6 (x1, x2, ODC3(BOp))) =
                                  (fn (y:AbsTypingTable) => DeclaredBool) 
...
fun ExpressionVCheck (EDC1 a) = (fn (b:AbsTypingTable) => b(a) <> NoDeclaration) |
    ...
    ExpressionVCheck (EDC4 (a1, a2, ODC1(AOp))) = (fn (b:AbsTypingTable) => true) |
